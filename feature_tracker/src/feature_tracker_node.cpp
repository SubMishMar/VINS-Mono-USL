...
FeatureTracker trackerData[NUM_OF_CAM];// An array of type FeatureTracker, this type is defined in feature_tracker.h
                                       // Since the number of cameras is 1, it's an array containing only one element
                                       // of type FeatureTracker
...
void img_callback(const sensor_msgs::ImageConstPtr &img_msg)
{
    ...
    // FREQuency control
    if (round(1.0 * pub_count / (img_msg->header.stamp.toSec() - first_image_time)) <= FREQ)//This condition means that two frames must be at least separated by
                                                                                            // 1 sampling time (i.e. 1/FREQ)
    {
        PUB_THIS_FRAME = true;
        // reset the FREQuency control
        if (abs(1.0 * pub_count / (img_msg->header.stamp.toSec() - first_image_time) - FREQ) < 0.01 * FREQ) // What does this condition imply?
        {
            first_image_time = img_msg->header.stamp.toSec();
            pub_count = 0;
        }
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < NUM_OF_CAM; i++)
        trackerData[i].readIntrinsicParameter(CAM_NAMES[i]);//takes the path of the config file where the intrinsic camera parameters are stored
    ...
    pub_img = n.advertise<sensor_msgs::PointCloud>("feature", 1000);//Point Cloud publisher
    pub_match = n.advertise<sensor_msgs::Image>("feature_img",1000);//Image with features marked on it,
                                                                    //still have to understand what the red and blue dots mean.
